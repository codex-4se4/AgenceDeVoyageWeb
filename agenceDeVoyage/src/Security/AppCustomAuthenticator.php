...
class AppCustomAuthenticator extends AbstractFormLoginAuthenticator implements PasswordAuthenticatedInterface
{
    ...
    public function getUser($credentials, UserProviderInterface $userProvider)
    {
        ...
        $user = $this->entityManager->getRepository(Utilisateur::class)->findOneBy(['login' => $credentials['login']]);
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token, $providerKey)
    {
        ...
        if ($token->getUser()->isVerified()!=1) {
            return new RedirectResponse($this->urlGenerator->generate('app_login'));
        }

        if ($token->getUser()->getRoles()[0] == "admin") {
